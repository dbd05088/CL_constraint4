...
class MemoryDataset(Dataset):
    ...
    @torch.no_grad()
    def get_batch(self, batch_size, stream_batch_size=0, use_weight=False, transform=None, twf=False, recent_ratio=None, exp_weight=False):
        ...
        if memory_batch_size > 0:
            if use_weight:
                ...
                indices = np.random.choice(range(len(self.images)), size=memory_batch_size, replace=False, p= weight / weight.sum())
